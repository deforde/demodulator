...
void apply_resampler_r(resampler_r_t* resampler, const real_data_t* const input, real_data_t* const output)
{
    ...
    if(downsampling_required) {
        ...
        stage_2_num_samples = (num_hist_samples + stage_1_num_samples) / decimation_factor * decimation_factor;
        const size_t samples_to_carry_over = (num_hist_samples + stage_1_num_samples) % decimation_factor;
        stage_2_data = (float*)malloc(stage_2_num_samples * sizeof(float));
        memcpy(stage_2_data, hist, num_hist_samples * sizeof(float));
        memcpy(stage_2_data + num_hist_samples, stage_1_data, (stage_2_num_samples - num_hist_samples) * sizeof(float));
        memcpy(hist, &stage_1_data[stage_1_num_samples - samples_to_carry_over], samples_to_carry_over * sizeof(float));
    ...
    apply_filter_r(&filter, decimation_factor, stage_2_data, stage_2_num_samples, &output->samples, &output->num_samples);
...
void apply_resampler_c(resampler_c_t* resampler, const iq_data_t* const input, iq_data_t* const output)
{
    ...
    if(upsampling_reqiured) {
        stage_1_num_samples = input->num_samples * interpolation_factor;
        stage_1_data = (float complex*)malloc(stage_1_num_samples * sizeof(float complex));
    ...
    if(downsampling_required) {
        ...
        stage_2_num_samples = (num_hist_samples + stage_1_num_samples) / decimation_factor * decimation_factor;
        const size_t samples_to_carry_over = (num_hist_samples + stage_1_num_samples) % decimation_factor;
        stage_2_data = (float complex*)malloc(stage_2_num_samples * sizeof(float complex));
        memcpy(stage_2_data, hist, num_hist_samples * sizeof(float complex));
        memcpy(stage_2_data + num_hist_samples, stage_1_data, (stage_2_num_samples - num_hist_samples) * sizeof(float complex));
        memcpy(hist, &stage_1_data[stage_1_num_samples - samples_to_carry_over], samples_to_carry_over * sizeof(float complex));
        resampler->num_historic_samples = samples_to_carry_over;
    }
    else {
        stage_2_data = stage_1_data;
    ...
    apply_filter_c(&filter, decimation_factor, stage_2_data, stage_2_num_samples, &output->samples, &output->num_samples);
