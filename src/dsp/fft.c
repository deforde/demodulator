...
void init_fft_c(fft_desc_c_t *fft, size_t len) {
  ...
  fft->plan = fftwf_plan_dft_1d(fft->len, fft->output, fft->output,
...
bool execute_fft_c(fft_desc_c_t *fft, const float complex *const iq_buf,
  ...
  if (num_samples < fft->len) {
    fprintf(stderr,
            "Cannot execute fft of length: %lu, on a input IQ data buffer "
            "containing only: %lu samples.\n",
            fft->len, num_samples);
    return false;
  }
  ...
  memcpy(fft->output, &fft->output[fft->len / 2],
         sizeof(fftwf_complex) * fft->len / 2);
  memcpy(&fft->output[fft->len / 2], fft->scratch,
         sizeof(fftwf_complex) * fft->len / 2);
...
void init_fft_r(fft_desc_r_t *fft, size_t len) {
  ...
  fft->output =
      (fftwf_complex *)fftwf_malloc(sizeof(fftwf_complex) * (fft->len / 2 + 1));
  fft->plan =
      fftwf_plan_dft_r2c_1d(fft->len, fft->input, fft->output, FFTW_ESTIMATE);
}

bool execute_fft_r(fft_desc_r_t *fft, const float *samples,
                   size_t num_samples) {
  if (num_samples < fft->len) {
    fprintf(stderr,
            "Cannot execute fft of length: %lu, on a input sample data buffer "
            "containing only: %lu samples.\n",
            fft->len, num_samples);
    return false;
  }
