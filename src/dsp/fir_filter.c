...
void init_filter_c(fir_filter_c_t* filter, const float* const taps, size_t num_taps)
{
    filter->taps = taps;
...
void apply_filter_c(fir_filter_c_t* filter, uint32_t decimation_factor, const float complex* const input, size_t input_len, float complex** output, size_t* output_len)
{
    ...
    const size_t num_output_samples = (size_t)(ceilf((float)(input_len + delay_line_num_samples - num_taps + 1) / decimation_factor));

    float complex* output_buf = (float complex*)malloc(num_output_samples * sizeof(float complex));
    *output = output_buf;
    *output_len = num_output_samples;

    for(size_t output_index = 0; output_index < num_output_samples; ++output_index) {
        ...
        for(int64_t input_index = input_start_index; input_index != input_end_index; ++input_index) {
            if(input_index < 0) {
                ...
                output_buf[output_index] += taps[filter_index] * crealf(delay_line[delay_line_index]) + taps[filter_index] * cimagf(delay_line[delay_line_index]) * I;
            }
            else {
                output_buf[output_index] += taps[filter_index] * crealf(input[input_index]) + taps[filter_index] * cimagf(input[input_index]) * I;
            }
            ++filter_index;
        }
    }
    ...
    *output_len = num_output_samples;
    ...
    for(size_t output_index = 0; output_index < num_output_samples; ++output_index) {
        ...
        for(int64_t input_index = input_start_index; input_index != input_end_index; ++input_index) {
            if(input_index < 0) {
                const size_t delay_line_index = input_index + delay_line_num_samples;
                output_buf[output_index] += taps[filter_index] * delay_line[delay_line_index];
            }
            else {
