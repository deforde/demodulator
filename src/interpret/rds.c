...
typedef enum { A, B, C_1, C_2, D } offset_word_label;
...
offset_word_t offset_words[] = {{.label = A, .word = 0xFC},
                                {.label = B, .word = 0x198},
                                {.label = C_1, .word = 0x168},
                                {.label = C_2, .word = 0x350},
                                {.label = D, .word = 0x1B4}};

pty_code_t pty_codes[] = {{.pty_code = 0, .pty_string = "None"},
...
bool crc_check(uint32_t block, offset_word_label *label) {
  ...
  while ((input & 0xFFFFFC00) != 0) {
    ...
    input ^=
        (polynomial << (sizeof(input) * 8 - current_shift - polynomial_len));
  }
...
void interpret_payload(uint16_t payload, offset_word_label label) {
  ...
  case A: {
    ...
    printf("RDS:\n"
           ...
           country_code, program_type, program_reference_num);
  ...
  case B: {
    ...
    printf("RDS:\n"
           ...
           "\t\tTraffic Program: %u\n",
           pty, (pty_code != NULL ? pty_code->pty_string : "Unknown"),
           group_type, group_version, traffic_program);
