...
void plot_amplitude_spectrum(float *ampl, size_t len) {
  if (DISPLAY_WIDTH > len) {
    // TODO: Provide implementation for case in which the source data len is
    // less than the display width
    fprintf(stderr,
            "Implementation for case in which the source data len (%lu) is "
            "less than the display width (%u) does not yet exist!\n",
            len, DISPLAY_WIDTH);
    return;
  }
  ...
  for (size_t i = 0; i < DISPLAY_WIDTH; ++i) {
    const size_t input_start_index =
        (size_t)fmin(floor(i * resample_ratio), (double)(len - 1));
    const size_t input_end_index =
        (size_t)fmin(ceil((double)(i + 1) * resample_ratio), (double)len);
  ...
  for (size_t i = 0; i < DISPLAY_WIDTH; ++i) {
    discretised_data[i] = (uint32_t)fminf(
        roundf(((resampled_data[i] - min_ampl) / ampl_range) * DISPLAY_HEIGHT),
        DISPLAY_HEIGHT - 1);
  }
  ...
  for (size_t i = 0; i < DISPLAY_HEIGHT; ++i) {
    int bytes_written = snprintf(&display_grid[i * NUM_COLUMNS],
                                 Y_AXIS_LABEL_LEN, "%+.2f", discretized_level);
    discretized_level -= discretized_ampl_res;
    memset(&display_grid[i * NUM_COLUMNS + bytes_written - 1], ' ',
           Y_AXIS_LABEL_LEN - bytes_written + 1);
    memset(&display_grid[i * NUM_COLUMNS + Y_AXIS_LABEL_LEN], ' ',
           NUM_COLUMNS - ROW_TRAILER_LEN - Y_AXIS_LABEL_LEN);
...
void do_plotting_c(float complex *samples, size_t num_samples) {
  ...
  if (num_samples < fft_len) {
    fprintf(stderr,
            "Number of samples (%lu) is less than the FFT length (%lu), cannot "
  ...
  for (size_t i = 0; i < num_ffts; ++i) {
    ...
    for (size_t i = 0; i < fft.len; ++i) {
      amplitude_spectrum[i] =
          (float)(10 * log10(sqrtf(fft.output[i][0] * fft.output[i][0] +
                                   fft.output[i][1] * fft.output[i][1]) +
                             DBL_MIN));
    }
