...
bool decode_diff_man(diff_man_decoder_t *decoder, uint32_t bit,
                     uint32_t *data_bit_out) {
  ...
  decoder->transition_history[decoder->num_transition_history_samples] =
      (uint32_t)transition;
  ++decoder->num_transition_history_samples;
  if (decoder->num_transition_history_samples ==
      sizeof(decoder->transition_history) /
          sizeof(*decoder->transition_history)) {
    size_t even_count = 0;
    size_t odd_count = 0;
