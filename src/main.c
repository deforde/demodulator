...
{
    printf(
        "usage: demodulator -f file -m modulation\n"
        "\t-f, --file source_data_file         A WAVE file containing the modulated source data\n"
        "\t-m, --modulation modulation_type    The type of modulation applied to the data contained in the source file (default: fm)\n"
        "\t                                    Supported: fm\n"
...
int main(int argc, char* argv[])
{
    ...
    if(strncmp(modulation_type, "fm", 2) != 0) {
        fprintf(stderr, "Illegal value: \"%s\" specified for modulation type. Currently only \"fm\" is supported.\n", modulation_type);
        return EXIT_FAILURE;
    }
