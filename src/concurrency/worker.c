...
void init_worker(worker_t *const worker, interconnect_t *const output,
                 thread_func_t func, void *thread_args) {
...
void wait_input(worker_t *worker) { interconnect_wait(&worker->input); }

void signal_output(worker_t *worker) { interconnect_signal(worker->output); }

void wait_output(worker_t *worker) { interconnect_wait(worker->output); }

void signal_input(worker_t *worker) { interconnect_signal(&worker->input); }

void lock_input(worker_t *worker) { interconnect_lock(&worker->input); }

void unlock_input(worker_t *worker) { interconnect_unlock(&worker->input); }

void lock_output(worker_t *worker) { interconnect_lock(worker->output); }

void unlock_output(worker_t *worker) { interconnect_unlock(worker->output); }
...
bool worker_read_input(worker_t *worker, void **data) {
  ...
  *data = NULL;
  ...
  // If no new data has been received, and the input is closed, then no new data
  // is ever going to be received, so return false
...
void worker_send_output(worker_t *worker, void **data) {
  ...
  // Check whether the output has already indicated that it is ready to receive
  // data
  lock_output(worker);
