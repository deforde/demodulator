...
void init_filter(fir_filter_t* filter, const float* const taps, size_t num_taps)
{
    filter->taps = taps;
...
void apply_filter(fir_filter_t* filter, uint32_t decimation_factor, const float* const input, size_t input_len, float** output, size_t* output_len)
{
...
void destroy_filter(fir_filter_t* filter)
{
